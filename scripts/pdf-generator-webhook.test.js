...
describe("pdf-generator / resolveFetchImageTags", () => {
  ...
  it("removes FETCH_IMAGE tags and does not perform network calls", async () => {
    global.fetch = jest.fn();
    expect(await resolveFetchImageTags("A [FETCH_IMAGE: term] B")).toBe("A  B");
    expect(global.fetch).not.toHaveBeenCalled();
  });

  it("removes markdown and html image payloads", async () => {
    const md = "Start ![diagram](https://example.com/a.png) <img src=\"https://example.com/a.png\" alt=\"a\"> End";
    expect(await resolveFetchImageTags(md)).toBe("Start   End");
  });
});
