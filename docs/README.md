...
- Ensure repo secrets include `APPWRITE_ENDPOINT`, `APPWRITE_PROJECT_ID`, `APPWRITE_API_KEY`, `GOTENBERG_URL`, `GOTENBERG_AUTH_TOKEN`, `GEMINI_API_KEY`, and `TAVILY_API_KEY` for solved-paper web context. You can also pass extra pdf-generator env keys via `APPWRITE_FUNCTION_ENV_PASSTHROUGH` (comma-separated key names).
