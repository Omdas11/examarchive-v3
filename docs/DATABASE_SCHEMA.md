...
| `users` | ⚠️ Connected with differences | 0 | ⚠️ 5/16 perfect, 11 different | collection existed; 0/22 missing attrs created; 11 attr definition mismatch(es); string diffs: email: type text → string, size undefined → 320 \| role: type text → string, size undefined → 64 (+9 more) |
