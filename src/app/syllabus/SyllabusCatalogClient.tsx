...
function PaperCard({
  ...
  const typeColors = paper.type ? TYPE_COLORS[paper.type] : undefined;

  /** Group question papers by year, sorted newest first. */
  const byYear = useMemo(() => {
    const map = new Map<number, Array<{ paperId: string; examType?: string }>>();
    for (const qp of paper.questionPapers) {
      const list = map.get(qp.year) ?? [];
      list.push({ paperId: qp.paperId, examType: qp.examType });
      map.set(qp.year, list);
    }
    return Array.from(map.entries()).sort((a, b) => b[0] - a[0]);
  }, [paper.questionPapers]);

  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-outline-variant/30 bg-surface p-4 shadow-sm transition-shadow hover:shadow-md">
      {/* Header row */}
      <div className="flex items-start justify-between gap-2">
        ...
        <Link
          ...
          className="shrink-0 rounded-lg bg-primary px-2.5 py-1 text-[11px] font-semibold text-on-primary hover:opacity-90 transition-opacity"
        >
          View Syllabus
        </Link>
      </div>
      ...
      <div>
        ...
        <p className="mt-0.5 text-xs text-on-surface-variant">{getSubjectDisplay(paper)}</p>
      </div>

      {/* Code, credits, lectures row */}
      <div className="flex flex-wrap items-center gap-2 text-[11px]">
        <span className="rounded-md bg-surface-container px-2 py-0.5 font-mono font-semibold text-primary ring-1 ring-outline-variant/30">
          {paper.paperCode}
        </span>
        {typeof paper.credits === "number" && (
          <span className="rounded-md bg-surface-container-high px-2 py-0.5 text-on-surface-variant">
            {paper.credits} cr
          </span>
        )}
        {paper.lectures && (
        ...
        {paper.university && (
          <span className="rounded-md bg-surface-container-high px-2 py-0.5 text-on-surface-variant truncate max-w-[120px]" title={paper.university}>
            {paper.university}
          </span>
        )}
      </div>
            ...
            {byYear.map(([year, items]) =>
              items.map((item) => (
                <Link
                  ...
                  className="rounded-full bg-primary/10 px-2.5 py-0.5 text-[11px] font-semibold text-primary hover:bg-primary/20 transition-colors"
                  title={item.examType ? `${year} · ${item.examType}` : String(year)}
                >
                  {year}{item.examType ? ` · ${item.examType.slice(0, 3).toUpperCase()}` : ""}
                </Link>
              )),
            )}
          </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-outline-variant/20">
        <a
          ...
          className="rounded-lg bg-surface-container-high px-2.5 py-1 text-[11px] font-semibold text-on-surface ring-1 ring-outline-variant/40 hover:bg-surface-container transition-colors"
        >
          Syllabus PDF
        </a>
        {pdfs.map((pdf) => (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-lg bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-200 hover:bg-emerald-100 transition-colors"
          >
        ...
        <header className="rounded-[28px] bg-surface p-6 shadow-sm shadow-primary/10 ring-1 ring-surface-container-high/40 space-y-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-base text-on-surface-variant">
              Browse curriculum by subject. Each card shows the syllabus details and available question papers by year.
            </p>
          </div>
          ...
          {!loading && !error && papers.length > 0 && (
            <p className="text-xs text-on-surface-variant">
              Showing {filteredPapers.length} of {papers.length} paper{papers.length !== 1 ? "s" : ""}
            </p>
          )}
        </header>
